...
class PandasDataAnalyst(BaseAgent):
    ...
    async def ainvoke_agent(self, user_instructions, data_raw: Union[pd.DataFrame, dict, list], max_retries: int = 3,
    ...
    def invoke_agent(self, user_instructions, data_raw: Union[pd.DataFrame, dict, list], max_retries: int = 3,
                     retry_count: int = 0, **kwargs):
...
def make_pandas_data_analyst(
    ...
    def invoke_data_wrangling_agent(state: PrimaryState):
        ...
        data_to_use = state.get("data_cleaned") if state.get("data_cleaned") else state.get("data_raw")
    ...
    def invoke_data_cleaning_agent(state: PrimaryState):
        """Invoke the data cleaning agent to clean raw data"""
        print("---INVOKING DATA CLEANING AGENT---")

        response = data_cleaning_agent.invoke({
            "user_instructions": state.get("user_instructions_data_cleaning"),
